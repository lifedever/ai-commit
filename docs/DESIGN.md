...
- 生成后显示元信息汇总（provider、模型、token 消耗、耗时）
- 自动检查版本更新（非阻塞，24 小时缓存）
...
| `AI_COMMIT_LANGUAGE` | commit message 和终端提示语言 | `en`（可选 `zh`） |
...
│   ├── update-check.ts   # 自动更新检查（非阻塞，24h 缓存）
│   ├── prompt.ts         # Prompt 模板管理
│   └── i18n.ts           # 国际化（中英文终端提示切换）
├── package.json
├── tsconfig.json
├── README.md
├── LICENSE
...
### 6. update-check.ts - 自动更新检查

```typescript
function checkForUpdate(localVersion: string): Promise<string | null>;
// 非阻塞检查 GitHub 上的最新版本
// 结果缓存 24h 在 ~/.ai-commit/.update-check
// 有新版本返回版本号，否则返回 null
// 请求超时 3 秒，失败静默返回 null
```
...
- emoji 支持（`-e` / `AI_COMMIT_EMOJI`）

### v1.2.0

- Claude Code 作为 provider（`-p claude` / `AI_COMMIT_PROVIDER`）
- Claude Code 自主读取项目源文件，生成更高质量的 commit message
