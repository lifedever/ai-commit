...
AI Commit 是一个命令行工具，通过分析 `git diff` 内容，调用大语言模型自动生成规范的 Git Commit Message。支持多种 LLM 提供商（DeepSeek、OpenAI、Anthropic 等兼容 OpenAI API 格式的服务），所有敏感配置通过环境变量管理。
...
| 变量名 | 说明 | 示例 |
|--------|------|------|
| `AI_COMMIT_API_KEY` | LLM 服务的 API Key | `sk-xxxxxxxx` |
